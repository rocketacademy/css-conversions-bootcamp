...
// hex <--> rgb <--> hsl
// node morecomfy-multi.js <from> <to>

// test cases
// node morecomfy-multi.js rgb hsl 'rgb(213,34,67)' 
// hsl(349deg,72.5%,48.4%)
// #d52243

const from = process.argv[2]
const to = process.argv[3]

const input = process.argv[4]; //now should be rgb input
let output;
...
if (from == to){
  ...
  if(to == 'hsl'){
    const intermediate = hexToRGB(input);
    output = RGBtoHSL(intermediate);
  }
} else if (from == 'rgb'){
  if(to == 'hex'){
    ...
    if(to == 'hsl'){
      output = RGBtoHSL(input);
    }
} else if (from == 'hsl'){
  if(to == 'rgb'){
    output = HSLtoRGB(input);
  }

  if(to == 'hex'){
    const intermediate = HSLtoRGB(input);
    output = RGBtoHex(intermediate);
  }
}
